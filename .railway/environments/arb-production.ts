// Imported without secret values on 2026-09-21. Preserve the complete environment.
...
export default defineRailway(() => {
  ...
  const arbScanner = service("arb-scanner", {
    ...
    source: github("Team-Win-and-Make-Monney/polymarket-arb-scanner", { branch: "master", checkSuites: true }),
  ...
  const polymarketEgressProxy = service("polymarket-egress-proxy", {
    build: { builder: "DOCKERFILE", dockerfilePath: "Dockerfile" },
    deploy: { restartPolicyType: "ON_FAILURE", restartPolicyMaxRetries: 5 },
    replicas: { "asia-southeast1-eqsg3a": 1 },
    networking: { tcpProxies: { "8080": {} } },
    env: { PORT: preserve(), PROXY_PASS: preserve(), PROXY_USER: preserve() },
  });

  return project("polymarket-arb-scanner", {
    resources: [arbScanner, polymarketEgressProxy, arbScannerVolume],
  });
});
