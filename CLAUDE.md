...
- Low-risk changes may use GitHub-native auto-merge after the branch is current with `master`, the final diff contains none of the stop conditions above, required `test` and `CodeRabbit` checks pass on the latest commit, every actionable review finding is resolved, and GitHub reports the PR mergeable. The ordinary Railway deployment triggered by that qualifying merge is authorized as part of this repository's autonomous lane; do not create a workflow that bypasses GitHub branch protection or polls around a missing required check.
