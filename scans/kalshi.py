...
def _is_catch_all_label(label: str | None) -> bool:
    """Whether a leg label is a bare catch-all such as "Other" or "Someone else"."""
    if not isinstance(label, str):
        return False
    return " ".join(label.lower().split()).strip(".,;:!?\"'") in _CATCH_ALL_LABELS


def _is_exhaustive_categorical(event_markets: list[dict]) -> bool:
    ...
    Returns:
        ...
        leg's rules text matches ``_NO_WINNER_RULE``.
    """
    if not any(_is_catch_all_label(m.get("yes_sub_title")) for m in event_markets):
        return False
    return not any(
        _NO_WINNER_RULE.search(f"{m.get('rules_primary') or ''} {m.get('rules_secondary') or ''}")
...
def scan_kalshi_multi(
    ...
    def _is_exhaustive_strike_ladder(event_markets: list[dict]) -> bool | None:
        ...
        categorical events (no strike fields); ``_is_exhaustive_categorical``
        judges those.
        """
    ...
    for event_ticker, markets in markets_by_event.items():
        ...
        if ladder_exhaustive is False:
            skipped_non_exhaustive += 1
            continue
        if ladder_exhaustive is None and not _is_exhaustive_categorical(markets):
    ...
    if skipped_categorical:
        logger.info(
            "Skipped %d categorical Kalshi events without a catch-all leg or with a no-winner rule "
            "(non-exhaustive sets).",
            skipped_categorical,
        )
    if skipped_blocked:
